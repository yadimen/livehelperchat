...
<?php foreach ($items as $file) : ?>
    <tr>         
        ...
        <td nowrap><a id="embed-button-<?php echo $file->id?>" onclick="return lhinst.sendLinkToEditor('<?php echo $chat->id?>','[file=<?php echo $file->id,'_',$file->security_hash?>]','<?php echo $file->id?>', '<?= htmlspecialchars($file->upload_name) ?>')" href="#" class="csfr-required btn btn-default btn-xs"><?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('file/list','Embed BB code');?></a></td>
    </tr>
<?php endforeach; ?>
</table>
