...
services.factory('LiveHelperChatFactory', ['$http','$q',function ($http, $q) {
    ...
    // chat/getnotificationsdata/(id)/1/5/8/7
            ...
            if (typeof data.items_processed == 'undefined') {
                ...
                angular.forEach(data.result, function(item, key) {
                    ...
                    if ( item.last_id_identifier ) {
                        ...
                        user_id_array += '/' + item.last_id_identifier;
                        chatsToNotify.push(item.last_id_identifier);
                ...
                if (/\d/.test(user_id_array)) {
                    LiveHelperChatFactory.getNotificationsData(user_id_array).then(function (data) {
                        angular.forEach(data, function (item, key) {
                            lhinst.playSoundNewAction(item.last_id_identifier,parseInt(item.last_id),(item.nick ? item.nick : 'Live Help'),(item.msg ? item.msg : confLH.transLation.new_chat), item.nt, item.uid);
                         //   console.log('last_id_identifier:'+item.last_id_identifier+'last_id:'+item.last_id+'nick:'+item.nick +'msg:'+ item.last_message);
                        });
